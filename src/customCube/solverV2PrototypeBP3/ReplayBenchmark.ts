// --- ReplayBenchmark (Solver v2 Primitive Prototype Sprint v3) -----------
// STEP 4/5: CycleChase vs Bounded Multi-Cycle Resolver (BP-1) vs
// Parity-Aware Cycle Breaker (BP-2) vs Non-Parity Structural Fix (BP-3),
// all unmodified, measured on the exact same real states. Mirrors BP-2's
// own ReplayBenchmark.ts shape (same SingleRunResult/BenchmarkSummary
// contracts) so this whole series stays directly comparable run over run.
...
import { tryNonParityStructuralFix } from "./NonParityStructuralFix";
...
export function runNonParityStructuralOn(snapshots: readonly FailureSnapshot[], lib: WingLibrary, deadlineMs: number): SingleRunResult[] {
  return snapshots.map((s) => runOne(s, (c, l, d) => tryNonParityStructuralFix(cloneCubies(c), l, d), lib, deadlineMs));
}
...
export interface BenchmarkSummary {
  ...
  avgPairDelta: number;
  avgTimeMs: number;
  cycleStructurePreservedRate: number; // fraction of activated runs where Pair did NOT decrease
}
