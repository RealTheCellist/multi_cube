...
export function evaluateReliabilityDecision(
  ...
  // Gate B already uses, measured against run1 as the baseline.
  const runtimeTolerance = runtimeRun2.p95Ms <= runtimeRun1.p95Ms * 1.15;
  const level2RuntimeStable = runtimeTolerance && runtimeRun1.deadlineMissCount === 0 && runtimeRun2.deadlineMissCount === 0;
  ...
  if (level1RegressionStable && level2RuntimeStable && level3ContractStable && level4DeterminismCharacterized) {
    ...
    rationale = `2회 독립 real population replay 간 flipRate=${(regressionTrend.flipRate * 100).toFixed(1)}% (<10%), Runtime p95 허용 범위 이내(deadlineMiss 0건 양쪽), Operating Contract 8개 전부 Closeout 대비 drift 없음, Determinism 실측 특성화 완료 -- Long-term Reliability를 확정하고 Continuous Validation Framework Sprint로 진행할 수 있다.`;
