...
// -- driver.
//   npx tsx src/customCube/runDeepCycleRefinementV1.ts
//
// No new Primitive designed. Only sweeps the existing Deep Cycle (BP-1)
// Prototype's (solverV2Prototype/BoundedResolver.ts's own real entry
// tryBoundedMultiCycleResolver()) Gate and Search Contract parameters via
// disclosed-duplicate parameterized copies (this Sprint's own
// deepCycleRefinementV1/ modules, reusing bridgeInjectionRefinementV1's own
// Search Contract simulator since it is the identical underlying DFS) --
// BoundedResolver.ts itself is never modified. Fast: direct per-case
// algorithm calls (~ms), not full solve() loops -- no long background run
// needed.
...
import { GATE_SWEEP_CONFIGS, BASELINE_GATE, tryBoundedMultiCycleConfigured, type GateConfig } from "./deepCycleRefinementV1/GateSweepSimulator";
...
import { buildCombinedTrial } from "./deepCycleRefinementV1/CombinedTrial";
import { evaluatePopulation, summarizeOutcomes, type EvaluationSummary, type CaseOutcome } from "./deepCycleRefinementV1/EvaluationRunner";
import { buildPairedComparison, runValidationFramework } from "./deepCycleRefinementV1/StatisticalValidation";
import { analyzeRootCause } from "./deepCycleRefinementV1/RootCauseAnalysis";

const DATA_DIR = "src/customCube/deepCycleRefinementV1/data";
const REPORT_PATH = `${DATA_DIR}/deep-cycle-refinement-v1-report.txt`;
const RESULT_JSON_PATH = `${DATA_DIR}/deep-cycle-refinement-v1-result.json`;
const DEADLINE_MS = 400; // matches PrototypeBenchmark.ts's own PROTOTYPE_DEADLINE_MS, same as Bridge Injection Refinement Sprint v1
...
function main() {
  ...
  console.log("Loading 142-case Hole Dataset (v4, unmodified) + Deep Cycle Primary Cluster labels (Blueprint Attribution Refinement Sprint v1, unmodified)...");
  const allCases = loadTargetPopulation();
  const { target, rest } = splitPopulation(allCases);
  console.log(`  target(Deep Cycle Primary Cluster)=${target.length}, rest=${rest.length}, total=${allCases.length}`);

  // STEP1: Baseline reproduction (real BoundedResolver.ts entry: BASELINE_GATE + BASELINE_SEARCH_CONTRACT)
  ...
  // STEP2: Gate Sweep (target population, holding Search Contract at baseline) -- each axis changed independently, no combined optimization
  console.log("STEP2: Gate Sweep...");
  const gateSweepResults = GATE_SWEEP_CONFIGS.map((cfg: GateConfig) => {
    const trial = tryBoundedMultiCycleConfigured;
  ...
  const bestGateEntry = gateSweepResults.find((r) => r.summaryTarget.configLabel === bestGate.configLabel)!;
  const bestGateConfig = bestGateEntry.config;
  console.log(`  Best Gate config: ${bestGateConfig.label}`);

  // STEP3: Search Contract Sweep (target population, holding Gate at baseline) -- reuses bridgeInjectionRefinementV1's own disclosed-duplicate DFS (identical mechanism)
  ...
  const bestSearchEntry = searchSweepResults.find((r) => r.summaryTarget.configLabel === bestSearch.configLabel)!;
  const bestSearchConfig = bestSearchEntry.config;
  ...
  console.log("STEP5: Statistical Validation (paired-diff, n=142 >= Directive's own N>=15 floor, Validation Framework)...");
  ...
  // STEP6: Refinement Decision (Level1-3) + Root Cause Analysis (Level4)
  ...
  const rootCause = analyzeRootCause({
    baselineGapRescueFull: baselineSummaryFull.gapRescueRate,
    bestGateOnlyGapRescueFull: bestGateEntry.summaryFull.gapRescueRate,
    bestSearchOnlyGapRescueFull: bestSearchEntry.summaryFull.gapRescueRate,
    combinedGapRescueFull: candidateSummaryFull.gapRescueRate,
  });
  const level4 = true; // Root Cause는 항상 명확히 분류됨(4개 카테고리 중 하나로 귀결) -- analyzeRootCause가 항상 하나의 rootCause를 반환하므로 "규명 실패"는 없음

  let decision: "A" | "B" | "C";
  if (rootCause.rootCause === "PRIMITIVE_LIMITATION") decision = "C";
  else if (level1 && level2 && level3) decision = "A";
  ...
  push("Solver Primitive Refinement Sprint #2 -- Deep Cycle Refinement Sprint v1 -- Report");
  push(`Generated: ${new Date().toISOString()}`);
  push(`Population: 142 cases (v4, unmodified) -- target(Deep Cycle Primary Cluster)=${target.length}, rest=${rest.length}`);
  push();

  push("STEP1. Baseline Reproduction (real BoundedResolver.ts entry tryBoundedMultiCycleResolver(), unmodified)");
  ...
  push("STEP5. Statistical Validation (paired-diff, n=142 -- deterministic mechanism, disclosed: population IS the N, exceeds Directive's own N>=15 floor)");
  push(
  ...
  push(`  Category C(Refinement)/prototype-stage Pipeline Decision: ${validation.pipelineResult.decision} -- ${validation.pipelineResult.decisionRationale}`);
  push();

  push("STEP6. Refinement Decision + Root Cause Analysis");
  push(`  Level1(Gap Rescue 증가, Δ>0): ${level1 ? "PASS" : "FAIL"}`);
  push(`  Level2(Regression 증가 없음): ${level2 ? "PASS" : "FAIL"}`);
  push(`  Level3(통계적 유의성, 95% CI 하한>0): ${level3 ? "PASS" : "FAIL"}`);
  push(`  Level4(Root Cause 규명): PASS(${rootCause.rootCause})`);
  push(`    Gate 단독 개선(full population Gap Rescue delta): ${(rootCause.gateOnlyDelta * 100).toFixed(2)}%p`);
  push(`    Search Contract 단독 개선(full population Gap Rescue delta): ${(rootCause.searchOnlyDelta * 100).toFixed(2)}%p`);
  push(`    결합 Candidate 개선(full population Gap Rescue delta): ${(rootCause.combinedDelta * 100).toFixed(2)}%p`);
  push(`    ${rootCause.explanation}`);
