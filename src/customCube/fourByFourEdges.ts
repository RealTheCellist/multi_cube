 ...
 *
 * No slow last-resort fallback: an earlier version added a raw-move BFS
 * for whichever restart left the smallest residual, but stress-testing it
 * directly (forcing frequent triggering) showed it's slow when actually
 * invoked (many seconds to tens of seconds) and not reliably successful
 * even then -- a worse experience than just failing fast, especially since
 * the fast path here has had zero observed failures across 1000+ trials
 * once the flip-trick primitive was added (see git history).
 */
